...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 16:28:05 2022
...
print(sys.getsizeof(df)) # 0.17 gb
...
#%% Drop columns and check size
df = df.drop(columns = ["SEX","MBSTATE_REC", "FAGECOMB"])
print(sys.getsizeof(df)) # 0.51 gb
df.reset_index(inplace = True)

#%% Define outcome variable
df["preterm"] = np.where(df["OEGest_R10"] <=5, True, False)

#%% Adjust race variable so it is consistent between years
df["MRACE6"] = df["MRACE6"].replace({10: 1, 20: 2, 30: 3, 40: 4, 
                                     41: 4, 51: 5, 61: 6})

#%% save file
#df.to_pickle("2016.pkl")
df.to_pickle("2016_v2.pkl")